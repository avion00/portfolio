...
/** Per-category presentation for the collection cards (title, year, hover tint). */
export interface CategoryMeta {
  /** Big display title shown on the card (e.g. "University Sports"). */
  title: string;
  /** Year shown on the right of the card header (mirrors the Works grid). */
  year: string;
  /** Hover-fill gradient — same delight as the project cells. */
  tint: [string, string];
}
...
// Display title, year and hover tint per category. (Static for now — wire to a
// backend later; new categories fall back to DEFAULT_META below.)
const CATEGORY_META: Record<string, CategoryMeta> = {
  ...
  Tour: { title: "Education Tour", year: "2024", tint: ["#15263a", "#0b1622"] },
  Friends: {
    title: "Friends & Crew",
    year: "2023",
    tint: ["#2c2114", "#17110a"],
  ...
  Solo: {
    title: "NYCA Nepal Organization",
    year: "2023",
    tint: ["#241a2e", "#130d1a"],
  },
  Other: { title: "More Moments", year: "", tint: ["#1f2023", "#141517"] },
};
...
/** Presentation for a category, falling back to the category name as title. */
export const categoryMeta = (category: string): CategoryMeta =>
  CATEGORY_META[category] ?? { ...DEFAULT_META, title: category };
